...
{
    CostmapAdapter adapter(rclcpp::get_logger("frontier_explorer_core_test"));
...
TEST(FrontierDetectorTest, DetectsSafeFreeCellsWithUnknownNeighbors)
{
    ...
    auto adapter = make_costmap(grid);
    FrontierDetector detector(0, rclcpp::get_logger("frontier_explorer_core_test"));

    const auto frontiers = detector.detect_frontier_cells(adapter);
...
TEST(FrontierDetectorTest, ClustersEightConnectedFrontiersAndComputesCentroids)
{
    ...
    auto clusters = detector.cluster_frontiers(adapter, cells);
...
TEST(FrontierPrunerTest, AppliesMapConstraintsRetryStateAndInset)
{
    ...
    FrontierPruner pruner(
        ...
        rclcpp::get_logger("frontier_explorer_core_test"));
...
TEST(FrontierScorerTest, NormalizesScoresAndAppliesPenalties)
{
    ...
    FrontierScorer scorer(weights, 2, rclcpp::get_logger("frontier_explorer_core_test"));
