...
bool CostmapAdapter::updateFromOccupancyGrid(const nav_msgs::msg::OccupancyGrid & map_msg)
{
    ...
    grid_map_.width = width;
    grid_map_.height = height;
    grid_map_.resolution = static_cast<double>(resolution);
    grid_map_.origin_x = map_msg.info.origin.position.x;
    grid_map_.origin_y = map_msg.info.origin.position.y;
...
const GridMap & CostmapAdapter::gridMap() const
{
    return grid_map_;
}
