  ...
  options.parameter_overrides(
      {
        ...
        rclcpp::Parameter("map_lifecycle_state_topic", topic_prefix + "/lifecycle_state"),
...
TEST_F(MapLifecycleNodeTest, PublishesOneSpecificFailureForRejectedSaveAttempt)
{
  ...
  const auto topic_prefix = "/map_lifecycle_failure_" + suffix;
  const auto temp_root =
    std::filesystem::temp_directory_path() / ("map_lifecycle_failure_" + suffix);
  std::filesystem::create_directories(temp_root);
  const auto invalid_directory = temp_root / "regular_file";
  std::ofstream(invalid_directory.string()) << "not a directory";

  auto client_node = std::make_shared<rclcpp::Node>("map_lifecycle_failure_client_" + suffix);
  auto lifecycle_node = std::make_shared<MapLifecycleNode>(
  ...
  auto state_subscription = client_node->create_subscription<MapLifecycleStateMsg>(
    topic_prefix + "/lifecycle_state",
    rclcpp::QoS(10).reliable().transient_local(),
    [&failure_details](const MapLifecycleStateMsg::SharedPtr message) {
      if (message->state == MapLifecycleStateMsg::FAILED) {
...
{
  const auto suffix = unique_suffix();
  const auto topic_prefix = "/map_lifecycle_success_" + suffix;
  const auto save_directory =
    std::filesystem::temp_directory_path() / ("map_lifecycle_success_" + suffix);

  auto service_node = std::make_shared<rclcpp::Node>("map_lifecycle_service_" + suffix);
  ...
  bool saved = false;
  MapLifecycleStateMsg::SharedPtr saved_message;
  ...
  auto state_subscription = service_node->create_subscription<MapLifecycleStateMsg>(
    topic_prefix + "/lifecycle_state",
    rclcpp::QoS(10).reliable().transient_local(),
    [&saved, &saved_message](const MapLifecycleStateMsg::SharedPtr message) {
      saved = saved || message->state == MapLifecycleStateMsg::SAVED;
      if (message->state == MapLifecycleStateMsg::SAVED) {
  ...
  ASSERT_NE(saved_message, nullptr);
  EXPECT_TRUE(saved_message->valid);
  EXPECT_DOUBLE_EQ(saved_message->unknown_ratio, 0.25);
  EXPECT_DOUBLE_EQ(saved_message->explored_ratio, 0.75);
  EXPECT_DOUBLE_EQ(saved_message->free_ratio, 0.5);
  EXPECT_DOUBLE_EQ(saved_message->occupied_ratio, 0.25);
  EXPECT_EQ(saved_message->width, 2U);
  EXPECT_EQ(saved_message->height, 2U);
  EXPECT_NEAR(saved_message->resolution, 0.05, 1e-6);
  EXPECT_FALSE(saved_message->saved_map_url.empty());
