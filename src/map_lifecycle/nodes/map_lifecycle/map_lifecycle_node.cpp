...
constexpr std::size_t kMapLifecycleStatePubDepth = 10U;
...
void MapLifecycleNode::declare_params()
{
  ...
  this->declare_parameter<std::string>(
    "map_lifecycle_state_topic",
    config_.map_lifecycle_state_topic);
  this->declare_parameter<std::string>(
    "final_map_topic",
...
void MapLifecycleNode::load_params()
{
  ...
  config_.map_lifecycle_state_topic =
    this->get_parameter("map_lifecycle_state_topic").as_string();
  config_.final_map_topic =
...
void MapLifecycleNode::apply_params()
{
  ...
  RCLCPP_INFO(
    logger_,
    "Map lifecycle params: map_topic=%s, state_topic=%s, auto_save=%s, "
    ...
    config_.map_lifecycle_state_topic.c_str(),
...
void MapLifecycleNode::create_interfaces()
{
  ...
  state_pub_ = this->create_publisher<MapLifecycleStateMsg>(
    config_.map_lifecycle_state_topic,
    rclcpp::QoS(rclcpp::KeepLast(kMapLifecycleStatePubDepth)).reliable().transient_local());
...
void MapLifecycleNode::publish_state(const std::string & detail)
{
  if (!state_pub_) {
    return;
  }
  ...
  MapLifecycleStateMsg msg;
  msg.stamp = this->now();
  msg.state = adapters::to_ros_map_lifecycle_state(core_.state());
  msg.state_text = map_lifecycle_state_to_string(msg.state);
  msg.valid = map_stats.valid;
  msg.detail = detail;
  msg.unknown_ratio = map_stats.unknown_ratio;
  msg.explored_ratio = map_stats.explored_ratio;
  msg.free_ratio = map_stats.free_ratio;
  msg.occupied_ratio = map_stats.occupied_ratio;
  msg.width = map_stats.width;
  msg.height = map_stats.height;
  msg.resolution = map_stats.resolution;
  msg.saved_map_url = core_.saved_map_url();
...
void MapLifecycleNode::map_callback(const nav_msgs::msg::OccupancyGrid::SharedPtr msg)
{
  ...
  publish_state("map_updated");
  if (core_.completion_detected() && !final_map_published_ && core_.map_statistics().valid) {
    publish_final_map("exploration_completed");
  }
...
void MapLifecycleNode::exploration_state_callback(const ExplorationStateMsg::SharedPtr msg)
{
  ...
  const auto event = adapters::to_core_exploration_completed_event(*msg);
  if (!event) {
    return;
  }

  core_.handle_exploration_completed(*event);
  RCLCPP_INFO(
    logger_,
    "Exploration completed event received: detail=%s",
    event->detail.c_str());
  publish_state(event->detail.empty() ? "exploration_completed" : event->detail);
  if (!final_map_published_ && core_.map_statistics().valid) {
    publish_final_map("exploration_completed");
  }
}
...
std::string MapLifecycleNode::map_lifecycle_state_to_string(std::uint8_t state) const
{
  switch (state) {
    case MapLifecycleStateMsg::EMPTY:
      return "EMPTY";
    case MapLifecycleStateMsg::ACTIVE:
      return "ACTIVE";
    case MapLifecycleStateMsg::READY:
      return "READY";
    case MapLifecycleStateMsg::SAVING:
      return "SAVING";
    case MapLifecycleStateMsg::SAVED:
      return "SAVED";
    case MapLifecycleStateMsg::FAILED:
      return "FAILED";
    default:
      return "UNKNOWN";
  }
