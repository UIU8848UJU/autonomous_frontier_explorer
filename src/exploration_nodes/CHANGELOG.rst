...
- `exploration_nodes` 现在只承载 FrontierStrategyNode、NavigationNode 及其 ROS/Nav2 适配节点。
...
Frontier 策略节点拆包
----------------------
- 将原 ``src/exploration`` 单包拆分为 ``frontier_strategy_core`` 与 ``exploration_nodes`` 两个 ROS 2 包。
- ``frontier_strategy_core`` 承载核心能力层，包括 ``costmap``、``detector``、``selector``、``scoring``、``types``、``geometry``、``reachability``、``utils`` 以及 ``FrontierGoalProvider``。
- ``exploration_nodes`` 承载 ROS 节点、BehaviorTree 插件、Nav2 reachability adapter、marker publisher、launch、config、rviz 和文档资源。
- 内部 include 路径统一迁移为 ``frontier_strategy_core/...`` 与 ``exploration_nodes/...``，移除旧的 ``core/...``、``nodes/...`` 等短路径依赖。
- ``exploration_nodes`` 通过 ``find_package(frontier_strategy_core REQUIRED)`` 链接核心库，形成 nodes 依赖 core 的单向依赖关系。
- BehaviorTree 插件库保留目标名 ``exploration_bt_nodes``，默认加载路径调整为 ``exploration_nodes`` 包下的 ``lib/libexploration_bt_nodes.so``。

Launch 与 Bringup 适配
----------------------
- ``exploration_nodes`` 继承原 ``exploration`` 的 ``frontier_strategy_node``、``navigation_node``、``exploration_bt_orchestrator_node`` 三个可执行入口，运行时 node 名、topic 和 service 名保持兼容。
- 更新 ``autonomousr_explorer_bringup`` 与 ``task_manager`` 中引用 frontier 节点的 launch 文件，ROS package 名从 ``exploration`` 改为 ``exploration_nodes``。
- ``autonomousr_explorer_bringup/package.xml`` 的运行依赖同步改为 ``exploration_nodes``。
- 原 ``exploration`` 包目录已移除，资源和文档迁移到 ``exploration_nodes``。
...
* 更新 ``doc/exploration_architecture.md``，加入 Mermaid 架构图，明确 ``TaskManagerNode``、``ExplorationBtOrchestratorNode``、BT 插件、``FrontierStrategyNode``、``FrontierGoalProvider``、``NavigationNode`` 和 Nav2 之间的边界。
...
0.0.6(2026-05-01)
------------------
* 将探索决策编排从 ``FrontierStrategyNode`` 中解耦，``FrontierStrategyNode`` 默认只作为 frontier 目标生成能力节点。
* 新增 ``FrontierGoalProvider`` 纯 C++ 能力类，承接地图/costmap、detector、pruner、scorer、selector、retry/blacklist 和 frontier goal 计算。
* 将 ``FrontierStrategyNode`` 收敛为 ROS wrapper，只负责参数、订阅、service、marker 和 state，不再持有 Nav2 action client。
...
* 将 ``frontier_strategy_node`` executor 改为 ``MultiThreadedExecutor``，避免 service 回调中等待 Nav2 planner action 结果时阻塞 action 回调处理。
...
* 废弃 ``enable_internal_navigation_loop`` 参数；即使配置为 ``true``，``FrontierStrategyNode`` 也不会主动发送 ``NavigateToPose``。
...
* 将 BT 节点拆成 BehaviorTree.CPP 动态插件库 ``libexploration_bt_nodes.so``，orchestrator 通过 ``bt_plugin_libraries`` 参数加载插件。
...
* 默认关闭 ``frontier_decision.enable_reachability_filter``，避免 FrontierStrategyNode 和 BT 同时做可达性过滤；FrontierStrategyNode 只负责候选生成和 marker/state。
...
* 新增 ``/exploration/state`` 和 ``/exploration_orchestrator/state`` 职责区分；继续发布旧 ``/exploration_state`` 作为兼容 topic。
* 整理源码目录：公共头文件迁移到 ``include/exploration``，源码迁移到 ``src``；BT 编排相关源码集中在 ``src/nodes/bt``。
* 清理 CMake：新增 ``frontier_strategy_core`` library，抽出 ``configure_exploration_target`` 复用 include path 和 ament dependencies。
* 统一内部 include 风格，通过 CMake 暴露 ``include/exploration``，源码内部使用 ``nodes/...``、``core/...`` 等短路径。
...
* 更新 ``frontier_strategy_node_doc.md``，补充 ``/map`` 与 ``/global_costmap/costmap`` 的职责分工、soft costmap scoring、marker 可视化策略和相关调试命令。
...
* 保留了保守策略的调参数值，后续可通过解除 ``bringup`` 里 ``exploration.yaml`` 注释掉的参数来转换。
* 重构参数结构，新增 ``FrontierStrategyParams`` 及 runtime/pruner/scorer/selection 分组。
...
0.0.2 (2026-04-04)
------------------
* 重构了frontier的代码，将 ``frontier_strategy_node`` 中的选择和计算解耦了出来。
* 新增了重试机制，重试次数限制。
* 改变了文件结构更易于后期维护。
...
0.0.1 (2026-04-02)
------------------
* 初始创建 ``exploration`` 独立 ROS 2 功能包。
* ``frontier_strategy_node`` 基础骨架搭建，拆分头文件与源文件。
