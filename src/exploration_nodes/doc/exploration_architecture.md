...
flowchart TD
    ...
    CFC -->|GetFrontierCandidates| FEN[FrontierStrategyNode]
...
- `FrontierStrategyNode` 只暴露 frontier 能力接口，不决定探索流程。
...
## FrontierStrategyNode 职责

`FrontierStrategyNode` 是探索能力 ROS wrapper，负责：
...
`FrontierStrategyNode` 不再包含内部导航循环，不主动发送 Nav2 goal；导航由 BT orchestrator 编排。
...
```bash
ros2 launch exploration_nodes exploration.launch.py
```
...
```text
include/exploration_nodes/nodes/
...
`navigation_core` 和 `robot_geometry_core` 不包含 ROS 头文件。ROS 消息、Nav2 costmap
以及 footprint 参数转换都位于适配层。
