...
`exploration` 位于 `src/exploration_nodes`，负责在 `/map`
...
`FrontierStrategyNode` 统一发布 `/exploration_state` 状态，并提供 frontier 能力服务。
...
| `/frontier_strategy_node/mark_frontier_failed` | `robot_interfaces/srv/MarkFrontierFailed` | 服务 | 外部导航失败后通知能力层更新 retry / blacklist。 |
| `/frontier_strategy_node/clear_frontier_blacklist` | `robot_interfaces/srv/ClearFrontierBlacklist` | 服务 | 清空 frontier blacklist。 |
| `/frontier_strategy_node/get_exploration_state` | `robot_interfaces/srv/GetExplorationState` | 服务 | 查询 Frontier 策略节点状态。 |

## 3. 核心流程

`FrontierStrategyNode` 不再有探索导航主循环。核心能力入口是
...
`FrontierPruner` 和 `FrontierStrategyPolicy`。
...
`FrontierStrategyPolicy` 是策略编排层和长期状态持有者：
...
策略评估结果会保留最近一轮评分候选，供 ROS provider 转换为 marker 和服务响应。
同时它会在日志里输出：
...
`FrontierStrategyNode` 只通过 `mark_frontier_failed` 接收失败事件。

## 6. 参数参考

参数文件：

- `src/exploration_nodes/config/frontier_strategy.yaml`
...
| `frontier_decision.enable_reachability_filter` | false | 是否在 FrontierStrategyNode 内部用 Nav2 `ComputePathToPose` 对评分靠前候选做可达性过滤；默认关闭，当前由 BT + NavigationNode 做可执行性过滤。 |
...
ros2 param get /frontier_strategy_node use_global_costmap_for_safety
```
