...
#include "exploration_nodes/nodes/frontier_marker_publisher.hpp"
...
using exploration_core::ExplorationStatus;
    ...
    /// @brief: FrontierStrategyNode 是 Frontier 策略的 ROS 适配节点，负责服务、话题和可视化，不负责导航编排
class FrontierStrategyNode : public rclcpp::Node
{
public:
    /// @brief: 构造 Frontier 策略能力节点
    /// @param options ROS2 节点选项
    explicit FrontierStrategyNode(
...
private:
    ...
    /// @param new_state 新状态
    /// @param detail 状态详情
    void set_state(ExplorationStatus new_state, const std::string & detail = {});

    /// @brief: 获取能力节点状态
    /// @return: 当前状态
    ExplorationStatus get_state() const;

    /// @brief: 将状态转换为字符串
    /// @param state 待转换状态
    /// @return: 状态字符串
    std::string state_to_string(ExplorationStatus state) const;
...
private:
    ...
    std::atomic<ExplorationStatus> state_{ExplorationStatus::IDLE};
