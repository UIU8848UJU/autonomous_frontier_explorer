    ...
    nav_msgs::msg::Path * path)
{
    ...
    if (wrapped_result.result) {
        ...
        response.path_length_m = static_cast<float>(navigation::navigation_core::pathLengthM(
            adapters::toCorePath(wrapped_result.result->path)));
        if (path != nullptr) {
...
{
    std::lock_guard<std::mutex> lock(footprint_costmap_mutex_);
    return path_safety_checker_.isSafe(
        adapters::toCorePath(path),
        footprint_costmap_.gridMap(),
        reason,
        max_path_cost);
}
