...
void FrontierExplorerNode::load_params()
{
    ...
    enable_internal_navigation_loop_ =
        this->get_parameter("enable_internal_navigation_loop").as_bool();
}
                                             ...
                                             const std::shared_ptr<const NavigateToPose::Feedback> feedback)
{
    ...
    if (dist_to_goal < params_.runtime.goal_reached_tolerance_m) {
        RCLCPP_DEBUG_WITH_CONTEXT(
            this->get_logger(),
            "Current frontier goal is within tolerance: remaining=%.3fm, tolerance=%.3fm",
            dist_to_goal,
            params_.runtime.goal_reached_tolerance_m);
    }
    set_state(ExplorationState::RUNNING);
...
FrontierGoalResult FrontierExplorerNode::compute_next_frontier_goal()
{
    FrontierGoalResult result;
    result.goal.header.frame_id = "map";
    result.goal.header.stamp = this->now();
    ...
    if (!map_msg_) {
        ...
        set_state(ExplorationState::RUNNING, result.reason_text);
        publish_state();
        return result;
    }
    ...
    set_state(ExplorationState::RUNNING, "SELECTING_FRONTIER");
    ...
    if (clusters.empty()) {
        ...
        set_state(ExplorationState::COMPLETED, result.reason_text);
        publish_state();
        return result;
    }
    ...
    if (!best_frontier.has_value()) {
            ...
            static_cast<std::size_t>(params_.runtime.max_frontier_failures))
        {
            ...
            set_state(ExplorationState::STUCK, result.reason_text);
            publish_state();
        }
        return result;
    }
...
{
    if (!enable_internal_navigation_loop_) {
        publish_state();
        return;
    }
    ...
    const auto result = compute_next_frontier_goal();
    if (!result.success || !result.goal_cell.has_value()) {
        return;
    }

    send_navigation_goal(result.goal_cell.value(), result.goal);
}
    ...
    std::shared_ptr<robot_interfaces::srv::MarkFrontierFailed::Response> response)
{
    ...
    if (!map_costmap_.isReady()) {
        response->success = false;
        response->message = "map costmap is not ready";
        return;
    }
    ...
    unsigned int row = 0U;
    if (!map_costmap_.worldToMap(
            request->failed_goal.x,
            request->failed_goal.y,
            col,
            row))
    {
        response->success = false;
        response->message = "failed goal is outside map";
        set_state(ExplorationState::STUCK, "MARK_FAILED_OUT_OF_MAP");
        publish_state();
        return;
    }
    ...
    response->retry_count = static_cast<uint32_t>(selector_.retry_count_for_goal(failed_cell));
    response->blacklisted = selector_.is_goal_blacklisted(failed_cell);
    response->success = true;
    response->message = response->blacklisted ? "frontier blacklisted" : "frontier failure recorded";
    ...
    RCLCPP_WARN_WITH_CONTEXT(
        this->get_logger(),
        "Marked frontier failed: row=%d col=%d reason=%u text=%s retry=%u blacklisted=%s",
        failed_cell.row,
        failed_cell.col,
    ...
    set_state(ExplorationState::RUNNING, "FRONTIER_FAILURE_RECORDED");
    publish_state();
}
...
{
    const auto cleared_count = selector_.clear_blacklist();
    response->success = true;
